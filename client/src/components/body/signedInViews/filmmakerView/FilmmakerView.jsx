...
class FilmmakerView extends React.Component {
  constructor(props) {
    ...
    this.refreshMyFilmsFeed = this.refreshMyFilmsFeed.bind(this);
  ...
  refreshMyFilmsFeed(e) {
    let username = this.props.userData.username;
    axios.get(`/getAllFilms?username=${username}`)
    .then(myFilms => {
      this.setState({
  ...
  render () {
    ...
    let refreshMyFilmsFeed = this.refreshMyFilmsFeed;
    ...
    return (
      <div id="filmmaker-view-wrapper">
        <div id="my-films-title">
          <span onClick={refreshMyFilmsFeed}>My Films</span>
        </div>
        <MyFilms
          fields={fields}
          myFilms={myFilms}
